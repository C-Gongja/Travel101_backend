...
// .orElseThrow(() -> new IllegalStateException("getTripByUuid User not
// found"));
...
// isFollowing =
// userFollowRepository.existsByFollowerIdAndFollowingId(currentUser.getId(),
// tripOwner.getId());
// }
...
// UserSnippetDto userSnippet = new UserSnippetDto(tripOwner.getUuid(),
// tripOwner.getName(),
// tripOwner.getUsername(), isFollowing);

// TripResponse response = new TripResponse(tripMapper.toDto(trip, currentUser),
// isEditable, userSnippet);
...
// .orElseThrow(() -> new IllegalStateException("getUserAllTrips User not
// found"));
// Long uid = user.getId();
...
// if (trip.isCompleted()) {
// int currentTripDays = (int) ChronoUnit.DAYS.between(trip.getStartDate(),
// trip.getEndDate()) + 1;
...
// updatedTrip.getEndDate().isBefore(today.plusDays(1))) {
// int updatedTripDays = (int)
// ChronoUnit.DAYS.between(updatedTrip.getStartDate(), updatedTrip.getEndDate())
// + 1;
...
// Set<Long> mediaIdsToKeep = new HashSet<>(); // 이번 업데이트에서 유지될 미디어 ID
// (S3TripMedia.id)
...
// Locations location = existingLocationsMap.getOrDefault(locDto.getNumber(),
// new Locations());
// location.setDay(day);
...
// .findByTripUidAndDayNumAndLocationNum(tripUid, dayDto.getNumber(),
// locDto.getNumber());

// // 1. 기존 미디어의 S3 키 맵 (빠른 조회를 위해)
// // Map<S3Key, S3TripMedia> 형태로 저장하여 ID를 쉽게 가져올 수 있도록 합니다.
// Map<String, S3TripMedia> existingS3KeyToMediaMap =
// existingMediaEntities.stream()
// .collect(Collectors.toMap(S3TripMedia::getS3Key, media -> media));

// // 2. 클라이언트에서 받은 DTO 미디어의 S3 키 세트
// Set<String> newMediaS3KeysFromClient = (locDto.getMedia() != null)
// ? locDto.getMedia().stream()
...
// s3KeysToAddNewOrUpdate.removeAll(existingS3KeyToMediaMap.keySet()); // 기존에 없는
// 키는 추가/업데이트 대상
...
// // 또는 mediaToDelete.forEach(media -> media.setLocationNum(null)); // 관계만 끊는
// 경우
// }
...
// .orElseThrow(() -> new RuntimeException("Media entry not found for S3 key: "
// + s3Key));
